    ...
    <div class="dashboard-content">
      ...
      <div v-else class="kpi-grid">
        ...
        <StatCard
          ...
          changeType="down"
          :icon="Warning"
          :accentColor="stats.assessmentsOverdue > 0 ? 'var(--cat-chart-red)' : 'var(--cat-chart-green)'"
        />
        <StatCard
          :title="t('dashboard.evidenceItems')"
        ...
        />
        <StatCard
          :title="t('dashboard.attestationsPublished')"
          :value="stats.totalAttestations"
          :change="0"
          changeType="up"
          :icon="Stamp"
          accentColor="var(--cat-chart-purple)"
        />
      </div>
      ...
      <div class="dashboard-grid">
        ...
        <!-- Assessment Pipeline (redesigned as stacked bars) -->
        <el-card class="dashboard-card">
          <template #header>
            <div class="card-header">
      ...
      <div class="dashboard-grid">
        <!-- Compliance Coverage by Standard (only standards with active projects) -->
        <el-card class="dashboard-card">
          <template #header>
            <div class="card-header">
      ...
      <el-card class="dashboard-card">
        ...
        <el-table v-else :data="recentAssessments" stripe role="grid" aria-label="Recent assessments table">
          <el-table-column prop="title" :label="t('assessments.titleField')" min-width="180"></el-table-column>
          ...
          <div v-else class="due-dates-list">
            ...
            <div v-for="item in upcomingDueDates" :key="item.id" class="due-date-item">
              <div class="due-date-content">
                ...
                <p class="due-date-meta">{{ item.project_name }} &middot; {{ formatDate(item.due_date) }}</p>
              </div>
              <el-badge :value="item.days_until_due + 'd'" :type="getDaysLeftColor(item.days_until_due)" />
            </div>
          </div>
        </el-card>
      </div>
...
import { FolderOpened, DocumentChecked, Odometer, Loading, Warning, Document, Stamp, CircleCheckFilled } from '@element-plus/icons-vue'
...
const maxDistCount = computed(() => {
  return Math.max(...assessmentDistribution.value.map(d => d.count), 1)
})
...
const renderConformanceChart = () => {
  ...
  const data = conformanceData.value
...
const renderPipelineChart = () => {
  ...
  const data = assessmentDistribution.value
  const style = getComputedStyle(document.documentElement)
  const textColor = style.getPropertyValue('--cat-text-secondary').trim() || '#8b949e'
  ...
  pipelineChartInstance = new Chart(pipelineChartCanvas.value, {
    type: 'bar',
    data: {
      labels: data.map(d => stateLabels[d.state] || d.state),
      datasets: [{
        data: data.map(d => d.count),
        backgroundColor: data.map(d => stateColors[d.state] || '#909399'),
        borderRadius: 4,
        barPercentage: 0.7,
